...
\begin{abstract}
    Introducing motion models into respiratory motion correction methods is a promising technique that can lead to a reduction in blurring and artefacts. However, the pool of research where motion modelling methods are applied to PET/CT is relatively shallow. Previous work has presented methods through which NAC TOF data can be used to fit motion models, not only to motion correct the volumes themselves, but also to warp a single attenuation map to the positions of the initial gated data. This work seeks to extend previous work to offer a comparison of respiratory motion correction methods, not only with and without motion models but also to compare pair-wise and group-wise registration techniques on simulation data with low counts, where the attenuation map is from a pseudo breath hold acquisition. To test the methods XCAT images are constructed, simulated and reconstructed without attenuation correction, then motion corrected using one of pair-wise, pair-wise with motion model, group-wise and group-wise with motion model registration. Next these motion corrected volumes are registered to the breath hold attenuation map. The PET data are then reconstructed using deformed attenuation maps and motion corrected. Evaluation compares the results of these methods, as well as against a non-motion corrected example, using a visual analysis, a comparison of profiles through a lesion inserted into the data as well as standard uptake values in lesions. Results indicate that the incorporation of a motion model and use of group-wise registration improves contrast and quantification.
\end{abstract}
